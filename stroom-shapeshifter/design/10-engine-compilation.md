...
| Reference resolution: `Refs` walks `RefExpression` parts every time | per-part dispatch, and **literal text is re-encoded to UTF-8 on every write** | `RefStrategy` classification: `SimpleLocal` / `SimpleRemote` / `LiteralBytes` (pre-encoded) / `Complex` | **closed** 2026-09-10 — change 3 classified them, design 25 phase 3 compiled capture bindings, design 30 phase 6 compiled the conditions' (E39, and `Refs` is deleted), and design 30 phase 5 made the store *behind* the reference a slot rather than a name (E44) |
