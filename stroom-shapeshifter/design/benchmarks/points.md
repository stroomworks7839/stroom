## What is owed — point 61, for an evening run

Point 61 against 60, `full`: `avro_users` up by more than half, everything else flat, and
`regex_lines` the verdict on the daytime artefact. Then the retired row by name, at 52 (the
interpreter), 60 and 61 — the reading design 39 §5 asked for, the retirement's cost and its
recovery on the same synthetic job:

```
engine-bench-points.sh full 523ed9c6ee 78146122f3
engine-bench-points.sh quick c710946def 523ed9c6ee 78146122f3   # then progressive by name, see below
```

The second is not the script's default rows: run `-p workload=progressive` at each of the
three shas with the benchmark's own annotations, on the same evening, and record all three
in 61's row. The script has no by-name mode; the three commands are

```
for sha in c710946def 523ed9c6ee 78146122f3; do (cd /home/dev1/engine-bench/wt-$sha && \
  ...
  -rf json -rff /home/dev1/engine-bench/$(date +%Y-%m-%d-%H%M)-$sha-progressive.json); done
```

## What was owed — points 59 and 60, settled overnight 2026-09-16/17

Run 23:56 to 00:51 on an idle box, `full`, three points from 58. Both keep; the readings are
in the rows. Where design 38 stands at 60:

| row | vs 52, the interpreter's last point | vs 50, the floor |
|---|---|---|
| `progressive_text` | **+14.6%** | **+22.5%** |
| `regex_lines` | +6.3 | **+12.9** |
| `ausearch` | +8.8 | +5.4 |
| `apache_httpd` | +5.4 | +6.5 |
| `element_storm` | +4.1 | +1.9 |
| `csv_header` | +2.8 | +1.9 |
| `log_sessions` | −1.3 | −1.5 |
| `win_sec` | −5.2 (inside its 7.9%) | +0.3 |
| `avro_users` | 246 ops/s, its own history from 54 | — |
| `progressive` | retired at 60 (D54); last read 238 at 53 against 470 at 52 | — |

Nothing is owed. The next point is whatever the next design builds.
...
| 44 | `9c77725d4e` | 2026-09-14 | Design 35 closes: plain assignment to a list is refused; `record_body` and its kin are the scalars they always were; no `__` names remain | **A control that closes the sequence.** What changed for the run rows is small and one-directional: eleven fixtures' per-record `variable` results — `record_body` in the `native` family, `query_body`, the `text_007_exact` families — were lists indexed at the current match and are scalars, so each is one slot assignment and one slot read per record where it was a positional put and an indexed get. `regex_lines` and `csv_header` are the rows that reach it (`004` has no such name; `001` does), and a hair **up** is the honest expectation, not a claim. `win_sec` and `win_sec_strict` are unchanged in shape — every declaration stays where it was — and every other row is flat by construction. The rest of this commit is names and a refusal, which no row can see. |
| 43 | `a699e5d002` | 2026-09-14 | The hand-written configurations name, scope and reduce their variables | **`apache_httpd` is the row, and this is the first point whose change is in a configuration rather than the engine.** Its 209 escape temporaries were run-lifetime *lists* — each `translate` a positional put at the current match into a list that grew for the run, each read an indexed get through `last` — and are 19 scalars now, declared on `log_line` so they are declared and restored per line: a slot assignment and a slot read where there was list growth and an accessor, and 244 of each per line. Expect **up**, and if it is not, the accessor was cheaper than it looks. `log_sessions` loses three list appends per request and the position ladder, so it may move a hair up. `ausearch`'s map is declared per record now — pushed and restored 32,109 times per operation where it was once — which is §11's cost in miniature and could show as a hair **down**; read it against the other rows before believing it. `win_sec` re-scopes 41 names from the source to `event_record`, a push per record over 41 slots where the source pushed once: the same question, on the row that is already the A/B. Everything else is flat. |
| 42 | `fd81619797` | 2026-09-14 | Design 35: `group()` moves into `Frames`; the registry holds the author's names and nothing else | **A control, with one row that can move.** `group()` was a registry slot bound per group through `declare` and `bindDense` — an undo-log entry, a list adopted into a slot and released on the pop — and is now a list the group frame holds, read as a frame field. Only a grouping reaches that path: **`log_sessions`** groups its rows and walks `group()` per group, so it may move a little, and up is the expected sign. Every other row is flat by construction; none groups. The compile rows may fall a hair — one fewer name interned per configuration — which is not worth reading. |
| 41 | `36e5a79c3e` | 2026-09-14 | Design 35 phase 5: `win_sec` rewritten under the model — 61 templates to 16 | **`win_sec` is the row, and it is a different workload from here on**, not a faster or slower version of the same one: the benchmark loads `projects/win_sec/project.json`, which now dispatches its fields level `strict` with a trailing eater and puts every labelled line into a map through a `seen` check, where it ran 57 unanchored searches per record. Read it against `win_sec_strict`, which keeps the 63-template configuration on the same input and the same output bytes: the two were the A/B on the search-versus-strict idiom, and this point turns `win_sec` into the A/B on *fifty-seven templates versus two plus a map*. Expect a large move either way, and read the direction: up says the searches were the cost; down says a `put` per line — key resolved, map hashed, `seen` consulted — costs more than an anchored regex that binds one scalar, which would be §11's ergonomics-versus-cost question answered by the benchmark. Every other row is flat by construction. |
| 40 | `ce776c7c72` | 2026-09-14 | Design 35 phase 4: the operation surface; design 16's constructs fold into declarations, mutations and accessors | **`log_sessions` is the row** — the sequence-and-fold workload: its folds are accessor parts now, its key is a map of lists built by an explicit walk, its `key-get` a `get` walked in place, its distinct values a set. Every store copies a collection, and the counter counts nested elements; on this row that is a few copies per record. `keys_lookup` and the other xmlbench cases are the same shape but not on the engine benchmark. **Expect flat to slightly up** on `log_sessions`. Two other rows changed shape: **`apache_httpd`**, whose 35 bare reads of list-declared names became `last` accessors — an accessor part resolves its collection and then answers, where a bare read was one slot read — and whose `value-map` is a declared table read by `get`; and **`ausearch`**, whose key-value reads are `get` parts. Both are reference-heavy rows, so a move there is the accessor's cost showing, not noise. Everything else is flat by construction: no other row's configuration has a collection operation in it. A `win_sec` move here is a finding. |
...
## What was owed — points 51 to 58, settled 2026-09-16 evening

Run 19:40 to 22:22 on a quiet box, `full`, nine points from 50. The readings are in the rows
above; the step-wise table:

| row | 51 | 52 | 53 | 54 | 55 | 56 | 57 | 58 |
|---|---|---|---|---|---|---|---|---|
| `apache_httpd` | +1.4 | −0.3 | +1.0 | +0.3 | −0.1 | +0.1 | +0.5 | **+4.9** |
| `ausearch` | −0.9 | −2.2 | +4.6 | −1.3 | −1.1 | **+6.1** | −0.6 | +0.3 |
| `csv_header` | −0.4 | −0.5 | +0.6 | −1.3 | +0.9 | 0.0 | +1.9 | +0.2 |
| `element_storm` | −0.5 | −1.6 | +2.3 | −1.1 | −0.2 | +0.3 | +0.1 | +2.7 |
| `log_sessions` | −0.8 | +0.6 | −1.6 | +1.7 | +0.1 | −1.8 | −0.1 | +1.7 |
| `progressive` | −0.3 | **+7.5** | **−49.4** | −0.8 | +5.1 | +0.1 | −1.0 | +2.7 |
| `progressive_text` | +1.8 | **+5.0** | **−8.7** | −0.6 | +1.0 | −1.7 | +1.0 | −1.2 |
| `regex_lines` | −0.3 | **+6.5** | −4.1 | +0.1 | +1.3 | +2.4 | −1.3 | **+5.1** |
| `win_sec` | **+6.6** | −0.8 | −0.3 | +1.6 | −1.4 | −5.2 | −0.3 | **+7.3** |
| `win_sec_strict` | +0.7 | −1.5 | +0.8 | +1.3 | −1.6 | −0.1 | −0.3 | +0.2 |
| `win_sec_xml` | −0.1 | +0.7 | +0.2 | −0.4 | +0.1 | −0.3 | +0.5 | −0.4 |

Design 37 closes on it: all four census splits keep (52, 56, 58 on their rows, 57 as a
tie), and its closing record is in design 37 §14. Design 38's two rows are the finding, not
the verdict; the census that followed and the fusion at 59 are in design 38 §7's phase-5
record.

## What is owed — points 59 and 60, overnight

Point 59, the fusion, against 58: `progressive_text` up by about a quarter, `regex_lines`
flat — the one row the daytime interleave could not clear, because the parts-path cuts were
in the same tree. Point 60, the cuts, against 59: the binary rows up a little and
`regex_lines` the verdict, as its row says. Launched 2026-09-16 night on an idle box:

```
engine-bench-points.sh full e04e634df4 417ca9e916 523ed9c6ee
```

## What was owed — design 37, settled 2026-09-15 evening

*Run 2026-09-15 18:24 to 20:01, six points on one boot, then two interleaves to settle what
the sequence could not: 46 against 47 (the control cost `ausearch` about 2% and nothing
else; the sequence's 5% dip was the box) and 48 against 49 (3d's losses on `log_sessions`
and `win_sec` were real, six of six). Point 50 is the split that followed, interleaved the
same night. The readings are in each point's row and in design 37 §5. Net against point 44
at point 50: `ausearch` about +17%, `csv_header` +7%, `regex_lines` about +5%, `win_sec` about
+6%, `apache_httpd` −1.4%, the rest flat — design 35's `regex_lines` debt repaid,
`progressive`'s untouched.*

Points 44 to 49 in one run, the benchmark's own fidelity (`full`): design 35's close as the
floor of the boot, the resolver restore, phase 2, phase 3b's control, phase 3c and phase 3d.
Point 44 was measured last night and is measured again because a reading only compares
within a boot. The design-only commits between them (`a2677c7c5a`, `160443c430`,
`af5acc5fd0`, `9fb703dff4`, `7466bfe7bc`) are records, identical in code to their
predecessors, and are not points. Nothing is scheduled; the run is launched by hand when the
box is quiet:

```
engine-bench-points.sh full 9c77725d4e 54a5dcdf94 7ddea06fbe 380898731f 6722af9951 da23082b75
```
...
record; the reading goes into design 37 §4 (phase 2), §5 (phases 3b, 3c and 3d) and §13.
...
control for point 41's `win_sec`.*

Points 36 to 44, in one run, the benchmark's own fidelity (`full`): the floor and the five
...
```
engine-bench-points.sh full 597274d25e f95a7287e1 a2fd4cf9f3 815caf7961 ce776c7c72 36e5a79c3e fd81619797 a699e5d002 9c77725d4e
```
