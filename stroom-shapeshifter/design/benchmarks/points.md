...
| 33 | `bd83c65dcc` | 2026-09-11 | The output sinks hold no Java collections: no element deque, no per-element lists, no scope map | **`element_storm` and `win_sec_xml` are the rows**, and `element_storm` is the interesting one: it has been **flat through every point in this sequence** — +2.2% at its best — because its work is writing elements and nothing until now touched that path. Allocation there is measured at **38,921,390 → 35,466,197 B/op, −8.9%**, so the question this point answers is whether that converts to throughput. `win_sec_xml` writes structure too and has moved for nothing so far. Everything else should be flat: a row that writes no elements cannot reach this code. |
...
```
engine-bench-points.sh full c2aee70a05 7335b0a7f2 2d8be8b5be 30581c3727 452ef6f6e5 b29f352bd6 350101e80b 4fc0d9c8d8 64232ecaf1 02a66fa760 f13b066fe4 bd83c65dcc
```
