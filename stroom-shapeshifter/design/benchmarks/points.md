...
| 30 | `64232ecaf1` | 2026-09-11 | A template's `only_match` is a sorted `int[]`, not a `Set<Integer>` | **The last hot collection, and the one with the clearest mechanism**: `Level` tested `Set.contains(matchCount)` on *every match*, hashing and boxing an `Integer` each time. `win_sec_strict` (569,199 matches/op) and `win_sec_xml` (244,054) are the rows, with `win_sec` (55,784) behind them. **But the honest caveat is that no workload in the corpus sets `only_match`** — the field is null for all of them — so what this removes on those rows is the null check's predecessor, not a scan. If they move, the cost was the `Set` field load and the branch, and that is a smaller claim than the match counts suggest. |
...
```
engine-bench-points.sh full c2aee70a05 7335b0a7f2 2d8be8b5be 30581c3727 452ef6f6e5 b29f352bd6 350101e80b 4fc0d9c8d8 64232ecaf1
```
