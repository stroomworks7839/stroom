...
| 54 | `ceb8f9cc90` | 2026-09-16 | Design 38 phase 5: the `avro_users` row — a real Avro container, written by the Avro library at test scope, parsed with no library | **A new row with no history; the engine is unchanged, so every existing row reads as 53.** The row is the match sequence, the binary casts and nested dispatch over raw bytes together, on 341,156 bytes of real container: header, metadata entries, 164 blocks of 100 length-prefixed records. Its daytime smoke is 180 ops/s, and the Rust prototype's native `apache-avro` path on the same file and the same output is 58 ops/s (design 38 §7's reading) — the ratio is the claim, the digits are tonight's. `progressive` keeps its history for now; whether the Avro row retires it is a ruling after this run. |
...
## What is owed — points 51 to 54, for an evening run

Points 50 to 54 in one run, the benchmark's own fidelity (`full`): the 3d split as this
boot's floor, the `equals` reshaping as a control, the `write` split, design 38's
retirement of the step interpreter, and the Avro row's first point. The commits between 50 and 51 are design records, the
...
```
engine-bench-points.sh full 228b7920e9 92f53652aa c710946def 2f56daf088 ceb8f9cc90
```

The readings go into points 51's to 54's rows, design 37 §8 and design 38 §8 (phase 5). The default run now has twelve rows: `ausearch_dispatch` stands beside `ausearch` from
