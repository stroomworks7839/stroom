...
## What is owed — point 59, and the parts-path cuts
...
in the same tree. Those cuts (`partsMatch` 347 → 309, `BinaryCasts.apply` 739 → 263,
`Level.length` unboxed) are a stash on the working tree, not a commit: +2% on the binary rows
in a clean daytime read against a possible −4% on `regex_lines` with no mechanism, and they
get a point of their own only after 59 has read.

```
engine-bench-points.sh full e04e634df4 417ca9e916
```
