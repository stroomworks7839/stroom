...
*Phase 4 is point 40 and joins the set; phase 5 — `win_sec` rewritten under the new model, 61
templates to 16 — gets its row when it is committed, because the benchmark's `win_sec` workload
loads `projects/win_sec/project.json` and that row changes shape with it. `win_sec_strict` keeps
the 63-template configuration and stays the A/B on the search-versus-strict idiom.*
...
```
engine-bench-points.sh full 597274d25e f95a7287e1 a2fd4cf9f3 815caf7961 ce776c7c72
```
