...
class VarRegistrySlotsTest {
    ...
    void namesFromTheDataGetSlotsOfTheirOwn() {
        ...
        assertThat(registry.get("alpha").get(1).asString()).isEqualTo("a");
        assertThat(registry.get("beta").get(1).asString()).isEqualTo("b");
    }
    ...
    void dataNamesDoNotLandOnACompiledSlot() {
        ...
        assertThat(registry.get(names.lookup("kept")).get(1).asString())
                .isEqualTo("survives");
    }
    ...
    void growingPastTheCompiledSizeKeepsEveryNameDistinct() {
        ...
        for (int i = 0; i < 100; i++) {
            assertThat(registry.get("k" + i).get(1).asString()).isEqualTo("v" + i);
        }
    }
}
