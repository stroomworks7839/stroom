...
final class JsonFields {
    ...
    static Dispatch readDispatch(final JsonValue node) {
        final JsonValue value = optional(node, "dispatch");
        return value == null ? null : lowercase(Dispatch.class, value.asString(), "dispatch mode");
    }

    static void writeDispatch(final JsonObject node, final Dispatch dispatch) {
        if (dispatch != null) {
    ...
    static Cast readCast(final JsonValue body) {
        final JsonValue value = optional(body, "as");
        return value == null ? null : lowercase(Cast.class, value.asString(), "cast");
    }

    static void writeCast(final JsonObject body, final Cast as) {
        if (as != null) {
    ...
    record Tagged(String name, JsonValue body) {
     ...
     */
    static Tagged tag(final JsonValue node, final String what) {
        if (node == null || node.isNull()) {
            throw new ConfigException("Missing " + what);
        }
        if (node.isString()) {
            return new Tagged(node.asString(), new JsonObject());
        }
        if (!node.isObject()) {
        ...
        final Map.Entry<String, JsonValue> property = ((JsonObject) node).entries().iterator().next();
        return new Tagged(property.getKey(), property.getValue());
    }

    static JsonObject wrap(final String name, final JsonValue body) {
        return new JsonObject().put(name, body);
    }
     ...
     */
    static void checkFields(final JsonValue node, final String what, final String... known) {
        if (node == null || node.isNull()) {
            return;
        }
        if (!node.isObject()) {
        ...
        for (final Map.Entry<String, JsonValue> entry : ((JsonObject) node).entries()) {
            if (!allowed.contains(entry.getKey())) {
                throw new ConfigException("Unknown field '" + entry.getKey() + "' in " + what);
            }
        }
    }
     ...
     * its owner.
     */
    static JsonValue required(final JsonValue node, final String field, final String what) {
        final JsonValue value = optional(node, field);
        if (value == null) {
    ...
    static JsonValue optional(final JsonValue node, final String field) {
        final JsonValue value = node == null ? null : node.get(field);
        return value == null || value.isNull() ? null : value;
    }

    /** A required text field: anything but a string node is refused by name. */
    static String text(final JsonValue node, final String field, final String what) {
        return string(required(node, field, what), field, what);
    }

    /** A bare text value: anything but a string node is refused by name. */
    static String text(final JsonValue node, final String what) {
        return string(node, null, what);
    }

    /** A text field, or null where it is absent or JSON null; anything else is refused by name. */
    static String optionalText(final JsonValue node, final String field) {
        final JsonValue value = optional(node, field);
    ...
    static int integer(final JsonValue node, final String field, final String what) {
        return whole(required(node, field, what), field, what);
    }

    /** A whole-number field, or the default where it is absent or JSON null. */
    static int integer(final JsonValue node,
                       final String field,
                       final String what,
                       final int fallback) {
        final JsonValue value = optional(node, field);
    ...
    static int integer(final JsonValue node, final String what) {
        return whole(node, null, what);
    }

    /** A required number field, whole or fractional. */
    static double number(final JsonValue node, final String field, final String what) {
        final JsonValue value = required(node, field, what);
        if (!value.isNumber()) {
            throw new ConfigException(
    ...
    private static String string(final JsonValue value, final String field, final String what) {
        if (!value.isString()) {
            throw new ConfigException(
    ...
    private static int whole(final JsonValue value, final String field, final String what) {
    ...
    private static String shape(final JsonValue value) {
        return value.shape();
    }

    /** A boolean field, false where it is absent or JSON null; anything else is refused by name. */
    static boolean flag(final JsonValue node, final String field) {
        return flag(node, field, false);
    }

    /** A boolean field, or the default where it is absent or JSON null; anything else is refused by name. */
    static boolean flag(final JsonValue node, final String field, final boolean fallback) {
        final JsonValue value = optional(node, field);
        if (value == null) {
            return fallback;
        }
        if (!value.isBoolean()) {
            throw new ConfigException("Expected true or false for '" + field + "', but was " + shape(value));
        }
        return value.asBoolean();
    }

    /**
     * A bare id value: the eight-four-four-four-twelve hexadecimal spelling, refused by name if
     * it is not one. Ids are text in the model — the client has no UUID class — and this is the
     * check that keeps them the shape everything downstream assumes.
     */
    static String uuid(final JsonValue node, final String what) {
        final String text = text(node, what);
        if (!isUuid(text)) {
            throw new ConfigException("Not a valid id for " + what + ": " + text);
        }
        return text;
    }

    /** A required id field. */
    static String uuid(final JsonValue node, final String field, final String what) {
        return uuid(required(node, field, what), what);
    }

    static boolean isUuid(final String text) {
        if (text.length() != 36) {
            return false;
        }
        for (int i = 0; i < 36; i++) {
    ...
    static void putIfPresent(final JsonObject node, final String field, final String value) {
        if (value != null) {
     ...
     */
    static <T> List<T> list(final JsonValue node, final String what,
                            final Function<JsonValue, T> reader) {
        if (node == null || node.isNull()) {
            return List.of();
        }
        if (!node.isArray()) {
        ...
        for (final JsonValue child : ((JsonArray) node).elements()) {
            values.add(reader.apply(child));
        }
        return values;
    }

    static <T> JsonArray array(final List<T> values, final Function<T, ? extends JsonValue> writer) {
