...
| `PollutedCorpusBenchmark` | `CorpusBenchmark`'s sixteen workloads and four methods, measured after `@Setup` has compiled and run the whole pattern corpus through both libraries | What a pipeline's JVM sees rather than a microbenchmark's: a JMH fork runs one pattern per JVM, which hands `java.util.regex` monomorphic profiles no real deployment gives it — the harness condition 06 §2 recorded against buffer `NETWORK`, and the many-patterns blind spot 06 §5 recorded as missing, closed together on 2026-09-03 (07 Phase 1). Read beside the clean row, not instead of it; neither is the headline alone |
