...
| `ByteMatcher`'s field set is a buffer-CSV coin — measured both ways | **Closed (2026-09-04) — no design; a gate instead** | August's ISSUES entry (old CPU): the tail-window machinery's three fields cost buffer CSV −12–14%, identical hot-path bytecode, two fork clusters. This machine, 2026-09-04: with D39's `contextEnd` back, stripping that machinery costs −16.8% (7,228 → 6,011, spread 1.01/1.00); D39's own deletion of one field had cost −3.2% bimodal and its restoration bought +13.7%. Three measurements, one mechanism: the scan plan's buffer-CSV loop sits in one of two JIT clusters (~7.2k / ~6.0–6.4k) keyed to the class's field set, and which set is fast is machine- and layout-specific. The rule replaces the design: every change to `ByteMatcher`'s fields is gated on buffer CSV, paired, before it lands. Evidence: `2026-09-04-*-p4strip-*` |
...
`RightToLeft`, Hyperscan and GNU grep) graduated to a phased plan on 2026-08-24, after the owner
...
(what only reverse fixes); and a key=value row shaped like the owner's text above, so the
...
the DS workload itself. Queued for the owner's ruling: whether Phase 4 proceeds, and if so
...
*Phase 4 ruled go by the owner (2026-08-24), ASCII-first v1 scope, and landed the same day.*
