...
| Phase 2's cost to the tree's lazy-run path | **Open (2026-09-03) — measured, undiagnosed** | Byte-compiling the tree's class nodes cost `LazyRunBenchmark.natural` BATCH_DOTALL 7.7% (1,542,434 → 1,422,990 ops/s), flat across every later phase and unmoved by the `splitsCharacter` fix, which is what makes it a separate item rather than part of it. The phase paid for itself elsewhere — tree `UNICODE` +9.0%, `TIER1_GREEDY` +8.1%, per-match `fixedwidth` +9.1%, `fancy` +10.4%, `EndAnchoredSearchBenchmark` BOUNDED +7–11% — so this is a redistribution, not a loss, and it is recorded rather than chased. Suspect by analogy with the gate above, unverified: `OneChar` now holds a `CharClass` and delegates, where it used to decode and test membership itself |
| Phase 3's cost to the per-match scan path | **Open (2026-09-03) — measured, undiagnosed** | Threading `ByteForm` through the compiled artifacts cost `PatternCorpusBenchmark.shapeshifter` weblog 6.3% (537,116 → 503,141 ops/s), and it is the residue that survives the gate fix (weblog sits at −7.3% against its pre-plan baseline, of which the gate accounted for only ~0.3 points here — notably less than the 2.9 points the diagnostic revert recovered, which is itself unexplained). Same suspicion as the row above and the same standing: recorded, not chased, until a workload's owner cares |
