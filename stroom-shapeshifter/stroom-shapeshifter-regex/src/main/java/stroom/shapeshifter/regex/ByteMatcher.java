...
public final class ByteMatcher {
     ...
     * buffer must blank the tail (as the engine's {@code InputWindow.fillAndBlankTail} does) or
     * pass an array that ends where its data ends; leaving it stale makes a legal empty match
     * at the region end fail, silently and according to what was there before.
     *
     * @param anchoring whether the match must begin at {@code from} or may be searched for.
     * @return true if a match was found; group accessors are then valid until the next call.
     */
