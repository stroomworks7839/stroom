...
/**
 * {@code host-name}: as {@code stroom.pipeline.xsltfunctions.HostName}. Impure: it resolves
 * through DNS, and a preview should not.
 */
public final class HostNameFunction extends StroomFunction {

    public HostNameFunction() {
        super("host-name", Signature.of(1, Kind.STRING, Kind.STRING, Kind.BOOLEAN), Purity.IMPURE);
    }
    ...
    public FunctionCall bind(final FunctionContext context) {
        return arguments -> {
            ...
            try {
                final String ip = arguments.string(0);
                if (ip == null) {
                    throw new IllegalArgumentException("Illegal non string argument found in function "
                                                       + name() + "() at position 0");
                }
                return TypedValue.of(InetAddress.getByName(ip).getHostName());
